...
//    }
    ...
    public DatabaseHelperImpl(final String dbPath) {
        mDbPath = dbPath;
    }

    static String getDbPath(Context context) {
        File fileDb = new File(context.getFilesDir(), DatabaseHelperImpl.DATABASE_NAME);
        return fileDb.getPath();
    }

    static String getSortDirection(int oldSort, int newSort, String direction) {
        if (oldSort == newSort) {
            if (direction.trim().equals(DatabaseHelperImpl.DESC.trim())) {
                return DatabaseHelperImpl.ASC;
            } else {
                return DatabaseHelperImpl.DESC;
            }
        } else {
            return DatabaseHelperImpl.ASC;
        }
